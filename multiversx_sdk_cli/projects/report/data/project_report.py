...
from multiversx_sdk_cli.projects.report.data.wasm_report import (
    WasmReport,
    merge_list_of_wasm_reports,
)
...
class ProjectReport:
    ...
    def get_rows_markdown(self, format_options: FormatOptions) -> List[List[str]]:
        ...
        if wasm_count == 0:
            ...
            return [
                [f" - {str(self.project_path / self.wasm_reports[0].wasm_name)}"]
                + self.wasm_reports[0].get_extracted_features_markdown(format_options)
            ]
        else:
            project_path_row = [f" - {str(self.project_path)}"]
            wasm_rows = [
                [f" - - {wasm.wasm_name}"] + wasm.get_extracted_features_markdown(format_options)
                for wasm in self.wasm_reports
            ]
