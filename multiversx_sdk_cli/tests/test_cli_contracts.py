...
def replace_variable_with_unknown_variable_for_adder():
    ...
    with open(
        parent / "testdata-out" / "SANDBOX" / "adder-bad-src" / "src" / "adder_bad_src.rs",
        "r",
    ) as f:
...
def test_contract_build():
    main(["contract", "build", "--path", f"{parent}/testdata-out/SANDBOX/adder"])
...
def test_bad_contract_build(capsys: Any):
    ...
    main(["contract", "build", "--path", f"{parent}/testdata-out/SANDBOX/adder-bad-src"])
